...
test( "Setting with input", function() {
  var el = $("<input value='red' />").spectrum();
  el.spectrum("show");

  var input = el.spectrum("container").find(".sp-input");
  var chooseButton = el.spectrum("container").find(".sp-choose");
  var cancelButton = el.spectrum("container").find(".sp-cancel");

  el.spectrum("show");
  input.val("blue").trigger("change");
  chooseButton.click();

  equal (el.spectrum("get").toHex(), "0000ff", "Color has been updated from text input");

  el.spectrum("show");
  input.val("#010203").trigger("change");
  console.log("setFromTextInput called", cancelButton.length);
  cancelButton.click();
  equal (el.spectrum("get").toHex(), "0000ff", "Color has not been updated when cancel is clicked");

  // el.spectrum("show");
  // input.val("green").change();
  // el.spectrum("hide");

  // equal (el.spectrum("get").toHex(), "0000ff", "Color has not been updated when picker is hidden");

  // el.spectrum("option", "clickoutFiresChange", true);
  // el.spectrum("show");
  // input.val("white").trigger("change");
  // el.spectrum("hide");

  // equal (el.spectrum("get").toHex(), "ffffff", "Color has been updated when picker is hidden with clickoutFiresChange = true");
